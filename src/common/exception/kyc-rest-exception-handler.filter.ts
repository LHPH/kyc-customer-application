...
@Catch()
export class KycRestExceptionHandler implements ExceptionFilter {
  ...
  constructor(private kycMessagesService: KycMessagesService) {}
    ...
    if (exception instanceof TypeORMError) {
      ...
      notification = this.kycMessagesService.getMessage(
        MessageCodes.UNAVAILABLE_OPERATION,
      );
      level = notification.type ?? level;
    } else if (exception instanceof KycRestException) {
      const restExc: KycRestException = exception;
      notification =
        restExc.params.message ??
        this.kycMessagesService.getMessage(MessageCodes.UNEXPECTED_ERROR);
      ...
      notification = this.kycMessagesService.getMessage(
        MessageCodes.UNEXPECTED_ERROR,
      );
    }

    const errorMessageToLogger = ExceptionUtil.getStringMessage(
      notification,
      status,
      exception,
    );
